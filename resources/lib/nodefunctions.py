    ...
    def parse_view( self, file, nodes, isFolder = False, origFolder = None, origPath = None, prefix = None ):
        ...
        try:
            ...
            if isFolder:
                ...
                nodes[ int( index ) ] = [ label, icon, origFolder, LANGUAGE( 32217 ), origIndex, mediaType ]
            else:
                # Check for a path
                path = root.find( "path" )
                if path is not None:
                ...
                if group is None:
                    # Add it as an item
                    nodes[ int( index ) ] = [ label, icon, origPath, LANGUAGE( 32218 ), origIndex, mediaType ]
                else:
                    # Add it as grouped
                    nodes[ int( index ) ] = [ label, icon, origPath, LANGUAGE( 32219 ), origIndex, mediaType ]
        except:
            print_exc()
